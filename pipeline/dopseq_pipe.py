...
if __name__ == '__main__':
    ...
    if conf['do_fastq_to_bam'] == 'True':
        command = [path_to_exec + "fastq_to_bam.py","-t",conf["target_genome"],"-c",conf["contam_genome"],
                    ...
                    "-p",conf["proc_bowtie2"], conf['fastq_F_file'],conf['fastq_R_file'],sample]
    ...
    if conf['do_bam_to_beds'] == 'True':
        ...
        run_script(command, run=True)
    if conf['do_control_stats'] == 'True':
        bed_basename = '.'.join([sample,target_name,'filter'])
        tsv_out = bed_basename + '.chrom.tsv'
        command = [path_to_exec + "control_stats.py",bed_basename,conf["sizes_file"]]
        run_script_to_file(command, tsv_out, run=True)
    if conf['do_control_plots'] == 'True':
        pos_bed = '.'.join([sample,target_name,'filter','pos','bed'])
        command = [path_to_exec + "control_plots.R",pos_bed,conf["sizes_file"]]
        run_script(command, run=True)
