...
def parse_command_line_arguments():
    ...
    parser = argparse.ArgumentParser(prog="compEDps")
    ...
    parser.add_argument("-a", "--pre-sort-by-name", action="store_true",
                        help="perform preliminary bam sorting by read name. Does not clean up.")
    parser.add_argument("-p", "--post-sort-index", action="store_true",
                        help="perform post sorting by coordinate and indexing of resulting bams. Does clean up.")
...
def sort_index(filename):
    srt_name = filename[:-4] + '.cs.bam' # ns for coordinate sorted   
    assert os.path.exists(srt_name) == False
