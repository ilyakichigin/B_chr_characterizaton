...
def main(args):    
    ...
    out_sam = '%s.%s_%s.sam' % (args.sample,reference,args.aligner) # simplified name: sample.genome.sam, 'ca' not included
    ...
    if (not os.path.isfile(out_sam)) or (os.path.getsize(out_sam) == 0):
        ...
        if args.aligner == 'bt2':
            ...
            command = '%s %s -1 %s -2 %s -x %s -S %s' % (args.path_to_aligner, args.aligner_args, f_ca_fq, r_ca_fq, reference_path, out_sam)
        elif args.aligner == 'bbm':
            assert os.path.isfile(args.reference_genome)
            command = '%s %s in1=%s in2=%s ref=%s out=%s' % (args.path_to_aligner, args.aligner_args, f_ca_fq, r_ca_fq, args.reference_genome, out_sam)
        else:
